...
(function (angular) {
  ...
  function isMarked(model, date) {
    //compare between the two dates
    //go through all the model dates and compares
    var there = false;
    if (model != undefined) {
      model.forEach(function (el) {

        var dec = el.date.getDate() == date.getDate() && el.date.getMonth() == date.getMonth() && el.date.getFullYear() == date.getFullYear();
        if (dec) {
          there = true;
        }
  ...
  Module.directive('datePicker', ['datePickerConfig', function datePickerDirective(datePickerConfig) {
    ...
    return {
      ...
      link: function (scope, element, attrs) {
        ...
        //this will set the dates when clicked

        //add a scope event that will add a mark that will be passed to the template called is marked
        ...
        //include the events marks ...
        scope.isMarked = function (date) {
          return isMarked(scope.marks, date);
        };
...
//          element.before(picker);
  ...
  angular.module("datePicker").run(["$templateCache", function ($templateCache) {
    ...
    $templateCache.put("templates/datepicker.html",
      ...
      "          <span\r" +
      "\n" +
      "            ng-class=\"{'marked':isMarked(day),'now':isNow(day),'active':isSameDay(day),'disabled':(day.getMonth()!=date.getMonth()),'after':isAfter(day),'before':isBefore(day)}\"\r" +
      "\n" +
      "            ng-click=\"clicked(day)\" ng-bind=\"day.getDate()\"></span>\r" +
      "\n" +
      "        </td>\r" +
      "\n" +
      "      </tr>\r" +
