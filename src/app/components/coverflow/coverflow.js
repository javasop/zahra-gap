...
angular.module('zahra.coverflow', []);

angular.module('zahra.coverflow').directive('zahraCoverflow', function(){
	return {
		restrict: 'E',
		replace: true,
      ...
      scope.$watch('images',function(newVal,oldVal){
        ...
        if(newVal)
        scope.coverflow.updateCovers(newVal,scope.coverflow);
...
Coverflow.prototype.init = function(){
	...
	// Images, currently if the image is pulled from the server, I think this doesn't work?
  if(this.images){
